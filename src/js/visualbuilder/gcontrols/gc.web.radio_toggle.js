...
var gc_web_radio_toggle = {
    ...
    "attributeDefinition": {
        "categories": [
            {"id": "main_props",
                ...
                "expanded": true,
                "properties": [
                    { "id": "name", "label": "Name:", "type": "value", "default": "fldRadioToggle", "propType": "input" },
    ...
    "createDefinition": function( component_id, container_id ) {
        var default_attributes = gc_factory.getDefaultAttributes( gc_web_radio_toggle.attributeDefinition );
        return {
            id: component_id,
            type: "radiotoggle",
    ...
    "renderDesign": function( component, gc_already_dropped ) {
        ...
        if (!component.attributes) {
            attributes = gc_factory.getDefaultAttributes( gc_web_radio_toggle.attributeDefinition );
        } else {
            attributes = gc_factory.migrateAttributes( gc_web_radio_toggle.attributeDefinition, component.attributes );
        }
        ...
        var instance_id = component.id,
            ...
            staticOptions = attributes.staticOptions ? attributes.staticOptions : [{ "displayValue": "Label", "dataValue": "", "disabled": false }],
        ...
        if (gc_already_dropped) {
            setTimeout('gc_web_radio_toggle.setSwitch('+instance_id+', \'' + size + '\', \'' + onLabel + '\', \'' + offLabel + '\', \'' + onColor + '\', \'' + offColor + '\', \'' + centralLabel + '\', \'' + centralIcon + '\', \'' +
            gc_web_radio_toggle._private.parseSize(centralWidth) + '\', \'' + gc_web_radio_toggle._private.parseSize(handleWidth) + '\')', 20);
        }

        var fragment_html = '<div id="' + component.id + '" gc-role="control" gc-type="radiotoggle" ' +
            'style="display:inline-block;position:relative;' + attributes.containerStyle.value + ';' + containerCss + '" ' +
            'class="' + gc_web_radio_toggle.default_classes + (attributes.containerClasses.value != '' ? ' ' + attributes.containerClasses.value : '') + '">';
        ...
        for (var i= 0, len = attributes.staticOptions.length; i<len; i++) {
            ...
            fragment_html +=
                ...
                '<input name="' + instance_id + '_field" type="radio" '+(staticOptions[i].disabled ? 'disabled="disabled"' : '')+'/>' +
                '</div>';
            '</div>'
        }
        fragment_html += '</div>';
        ...
        var component_instance = {
            ...
            "callback": function(comp_inst) {
                if (! gc_already_dropped) {
                    setTimeout(gc_web_radio_toggle.setSwitch(comp_inst.id, size, onLabel, offLabel, onColor, offColor, centralLabel, centralIcon,
                            gc_web_radio_toggle._private.parseSize(centralWidth), gc_web_radio_toggle._private.parseSize(handleWidth)), 20);
                }
            }
        };
    ...
    "loadPropertyPanel": function(gc_component_definition) {
        ...
        $( '#gc_radiotoggle_attr_id').val( gc_component_definition.id );
        gc_factory.generatePropertyPanel( gc_web_radio_toggle.attributeDefinition, gc_component_definition );
    ...
    "savePropertyPanel": function() {
        var id = $( '#gc_radiotoggle_attr_id').val();

        var attributes = gc_factory.getPropertiesFromPanel( gc_web_radio_toggle.attributeDefinition );
        ...
        $('#'+id).attr( {
            'class': gc_web_radio_toggle.default_classes + ' ' + attributes.containerClasses.value,
            'style': 'display:inline-block;position:relative;' + attributes.containerStyle.value + ';' + containerCss
        });
        ...
        if (dynamicOptions) {
            fragment_html +=
                ...
                '<input name="' + id + '_field" type="radio">' +
                '</div>';
        } else {
            for (var i=0; i<attributes.staticOptions.length; i++) {
                ...
                fragment_html +=
                    ...
                    '<input name="' + id + '_field" disabled="disabled" type="radio" '+(attributes.staticOptions[i].disabled ? 'disabled="disabled"' : '')+'>'+
                    '</div>';
            }
        }
