...
var gc_web_slider = {
    ...
    "createDefinition": function( component_id, container_id ) {
        var default_attributes = gc_factory.getDefaultAttributes( gc_web_slider.attributeDefinition );
        return {
            id: component_id,
            type: "slider",
    ...
    "renderDesign": function( component ) {
        ...
        if (!component.attributes) {
            attributes = gc_factory.getDefaultAttributes( gc_web_slider.attributeDefinition );
        } else {
            attributes = gc_factory.migrateAttributes( gc_web_slider.attributeDefinition, component.attributes );
        }
        ...
        if (! attributes.css.width) attributes.css.width = 200;
        if (! attributes.css.height) attributes.css.height = 200;
        ...
        if (attributes.orientation.value == 'horizontal') {
            ...
            style += ';height:'+height+'px;margin:20px auto';
            divStyle = 'min-height:50px;height:auto;width:50px;display:inline-block;';
        }
        ...
        var fragment_html = '<div id="' + component.id + '"  gc-role="control" gc-type="slider" ' +
            ...
            'class="' + gc_web_slider.default_classes + (attributes.containerClasses.value != '' ? ' '+attributes.containerClasses.value : '') + '">';
    ...
    "loadPropertyPanel": function(gc_component_definition) {
        // default component width and height
        if (! gc_component_definition.attributes.css.width) gc_component_definition.attributes.css.width = "200";
        if (! gc_component_definition.attributes.css.height) gc_component_definition.attributes.css.height = "200";

        $( '#gc_slider_attr_id').val( gc_component_definition.id );
        gc_factory.generatePropertyPanel( gc_web_slider.attributeDefinition, gc_component_definition );
    ...
    "savePropertyPanel": function() {
        ...
        var attributes = gc_factory.getPropertiesFromPanel( gc_web_slider.attributeDefinition );
        ...
        el.attr( {
            'class': gc_web_slider.default_classes + ' ' + attributes.containerClasses.value,
            'style': 'display:inline-block;position:relative;' + attributes.containerStyle.value + ';' + containerCss
        });
        ...
        var dynamicProps = {range: {min: Number(attributes.min.value), max: Number(attributes.max.value)}},
            ...
            width = parseInt(attributes.css.width),
            height = parseInt(attributes.css.height),
            bgcolor = attributes.css.background || "#FAFAFA",
            fgcolor = attributes.css.color || "#3276b1",
            handleColor = attributes.css.handleColor || "#FFF",
        ...
        if (orientation == 'horizontal') {
            ...
            addStyle = ';width:'+width+'px;margin:0 auto';
            divStyle = 'min-height:50px;height:auto;width:'+(width)+'px;';
        } else {
            ...
            addStyle = ';height:'+height+'px;margin:20px auto';
            divStyle = 'min-height:50px;height:auto;width:50px;display:inline-block;';
        }
        //console.log(JSON.stringify(dynamicProps))
