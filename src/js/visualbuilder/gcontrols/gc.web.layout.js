...
var gc_web_layout = {
    ...
    "renderDesign": function( component_attributes, container_id, child_index ) {
        ...
        var fragment_html = '<div id="' + container_id + '_layout_'+child_index+'"  gc-control-id="'+container_id + '" gc-role="control-child" gc-type="layout" gc-child-index="'+child_index+'"'
            + ' style="min-height:10px;height:auto;">';

        for (i=0; i<component_attributes.rows.length; i++) {
            fragment_html += gc_web_layout_row.renderDesign(component_attributes.rows[i], container_id, i, child_index);
        }
    ...
    "loadPropertyPanel": function(gc_component_definition, gc_control_child_id) {
        ...
        fragment_html += '<div class="row"><a id="layout_manager_add_row" href="javascript:void(0);" container-id="'+gc_component_definition.id+'"><span class="fa fa-plus-circle" style="margin-right: 5px"></span>Add row</a></div>';
        ...
        $('#layout_manager_add_row').click( function() {
            ...
            gc_web_layout.addRow(container_id);
        });
    },
