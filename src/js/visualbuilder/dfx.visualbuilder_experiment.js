...
DfxVisualBuilder.removeNotOverriddenAttributes = function (updated_attributes, gc_type, attr_full_path) {
    ...
    var removeNotOverriddenArrayTypeAttribute = function (updated_array_attribute) {
        for (var i = 0; i < updated_array_attribute.length; i++) {
            removeNotOverridden( updated_array_attribute[i] );
        }
    };
    ...
    var removeNotOverridden = function (updated_attributes, attr_full_path) {
        for (var attribute in updated_attributes) {
            if (updated_attributes.hasOwnProperty(attribute)) {
                ...
                //if (attribute != 'value' && attribute != 'status') {

                    if ( Array.isArray(updated_attributes[attribute]) ) {
                        //removeNotOverriddenArrayTypeAttribute( updated_attributes[attribute] );
                        //
                        // must send $index and full path to overrideAttribute() like below:
                        // <input class="col-md-12" type="text" ng-model="row.style.value" style="margin-left:-15px;" ng-change="overrideAttribute('layout')"/>
                    } else {
