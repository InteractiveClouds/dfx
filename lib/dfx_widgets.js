...
Widget.createCompiledView = function (req, callback) {
...
Widget.createViewZip = function (view_name, view_data, callback) {
...
Widget.download_view = function (req, callback) {
    ...
    mdbw.getOne(repositoryPrefix + req.session.tenant.id, 'datawidgets', {
        ...
        Widget.createCompiledView(data.src, function (cards) {
            var view_data = {
                "src": {
                    "name": data.name+'.json', 
                    "value": data.src
            ...
            Widget.createViewZip(data.name, view_data, function (view_zip_path) {
...
Widget.getViewArchive = function (req, res) {
    ...
    if (!req.query.path) {
        ...
        fs.exists(view_zip_path, function (exist) {
            if (exist) {
                res.download(view_zip_path, view_zip_name, function(err){
                    var view_folder_path = view_zip_path.replace('.zip', '');
                    fs.unlink(view_zip_path);
                    QFS.removeTree(view_folder_path);
                });
            } else {
