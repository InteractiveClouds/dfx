...
compiler.deployAppBuildNew = function (req, res) {
    ...
    var o = {
        path: path.join(__dirname, '..', APP_BUILD_DIR, req.session.tenant.id, buildVersion, 'app',  buildVersion + '.zip'),
    ...
    unzip.decompress(o)
        .then(function(){
            ...
            o.path = path.join(__dirname, '..', APP_BUILD_DIR, req.session.tenant.id, buildVersion, 'application_menus',  buildVersion + '.zip');
        ...
        .then(function(){
            ...
            o.path = path.join(__dirname, '..', APP_BUILD_DIR, req.session.tenant.id, buildVersion, 'application_configuration',  buildVersion + '.zip');
            return unzip.decompress(o);
        })
        .then(function(){
            delete o.dest_path;
            o.folder = APP_FSDB_DIR;
            o.type = "dataqueries";
            o.path = path.join(__dirname, '..', APP_BUILD_DIR, req.session.tenant.id, buildVersion, 'dataqueries',  buildVersion + '.zip');
            return unzip.decompress(o);
        })
        .then(function(){
            o.folder = APP_FSDB_DIR;
            o.type = "users";
            o.path = path.join(__dirname, '..', APP_BUILD_DIR, req.session.tenant.id, buildVersion, 'users',  buildVersion + '.zip');
            return unzip.decompress(o);
        })
        .then(function(){
            o.folder = APP_FSDB_DIR;
            o.type = "roles";
            o.path = path.join(__dirname, '..', APP_BUILD_DIR, req.session.tenant.id, buildVersion, 'roles',  buildVersion + '.zip');
            return unzip.decompress(o);
        })
        .then(function(){
            o.folder = APP_FSDB_DIR;
            o.type = "metadata";
            o.path = path.join(__dirname, '..', APP_BUILD_DIR, req.session.tenant.id, buildVersion, 'metadata',  buildVersion + '.zip');
            return unzip.decompress(o);
        })
        .then(function(){
            o.folder = APP_FSDB_DIR;
            o.type = "auth_providers";
            o.path = path.join(__dirname, '..', APP_BUILD_DIR, req.session.tenant.id, buildVersion, 'auth_providers',  buildVersion + '.zip');
        ...
        .then(function(){
            o.folder = APP_FSDB_DIR;
            o.type = "db_drivers";
            o.path = path.join(__dirname, '..', APP_BUILD_DIR, req.session.tenant.id, buildVersion, 'db_drivers',  buildVersion + '.zip');
            o.dest_path = path.join(__dirname, '..', o.folder, o.type);
            return unzip.decompress(o);
        })
        .then(function () {
