...
var api = {
    ...
    delete: function (parsed) {
        ...
        Query.deleteQuery(parsed.queryParameters.queryName, parsed.req, function () {
            return D.resolve("API Service Object " + parsed.queryParameters.queryName + " was successfully deleted!");
    ...
    deleteItem: function (parsed) {
        ...
        Query.deleteQueryItem(parsed.queryParameters, parsed.req, function (err) {
            if (!err) {
                return D.resolve("API Route item " + parsed.queryParameters.apiRouteItemName + " was successfully deleted!");
            } else {
                return D.reject(err);
            }
        });
    ...
    updateItem: function (parsed) {
        ...
        Query.updateQueryItem(parsed.queryParameters, parsed.req, function (err) {
            if (!err) {
                return D.resolve("API Route item " + parsed.queryParameters.newApiRouteItemName + " was successfully update!");
            } else {
                return D.reject(err);
            }
        });
    ...
    update: function (parsed) {
        ...
        Query.set(parsed.queryName, parsed.queryParameters, parsed.req, function (err, data) {
            return err
                ? D.reject(err)
                : D.resolve("API Route " + parsed.queryName + " has been updated successfully!");
        });
        return D.promise;
    },
    ...
    createCategory: function (parsed) {
        ...
        Query.createNewCat(parsed.queryParameters, parsed.req, function (err, data) {
            return err
                ? D.resolve(err)
                : D.resolve('The API Route category was created successfully!');
        });
        return D.promise;
    },
    ...
    updateCategory: function (parsed) {
        ...
        Query.setCat(parsed.name, parsed.queryParameters, parsed.req, function (err, data) {
            return err
                ? D.resolve(err)
                : D.resolve('The API Route category has been updated successfully!');
        });
        return D.promise;
    },
    ...
    removeCategory: function (parsed) {
        ...
        Query.deleteCat(parsed.queryParameters, parsed.req, function () {
            return D.resolve('The API Route category ' + parsed.queryParameters.name + ' has been deleted successfully!');
        });
        return D.promise;
    },
    ...
    createService: function (parsed) {
        ...
        Query.createNewService(parsed.queryParameters, parsed.req, function (err, data) {
            return err
                ? D.resolve(err)
                : D.resolve('The API Route service  was created successfully!');
        });
        return D.promise;
    },
    ...
    updateService: function (parsed) {
        ...
        Query.setService(parsed.name, parsed.queryParameters, parsed.req, function (err, data) {
            return err
                ? D.resolve(err)
                : D.resolve('The API Route service  has been updated successfully!');
        });
        return D.promise;
    },
    ...
    removeService: function (parsed) {
        ...
        Query.deleteService(parsed.queryParameters.serviceName, parsed.req, function () {
            return D.resolve('The API Route service ' + parsed.queryParameters.serviceName + ' has been deleted successfully!');
        });
        return D.promise;
    },
            ...
            .then(function(data){
                if (data.length > 0) {
                    ...
                    async.each(data, function (val,cb) {
                            if (val) {
                                return D.resolve("API route with name - " + parsed.queryParameters.apiRoutes[counter] + " is already exists in application " + parsed.queryParameters.app);
                            }
                            cb();
                            counter++;
                        },
...
Query.executeApiRoute = function (req, res) {
    ...
    var tenantid = req.user.tenantid || req.user.id,
        ...
        parameters = (req.method == 'POST') ? req.body.params : req.query,
...
Query.createNew = function (queryParameters, req, callback) {
    ...
    Q.all(tasks).then(function (response) {
        response.forEach(function (res) {
            if (res) {
                 callback('One of service url is already exists', null);
                 err = true;
            }
        })
    }).then(function(){
