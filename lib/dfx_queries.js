    ...
    Query.validateServiceUrl = function(req, url, appName, uuid) {
        ...
        if ((!url) || (!result) || (result.index != 0)) {
            ...
            mdbw.exists(DB_TENANTS_PREFIX + req.user.tenantid, 'dataqueries', query).then(function(exist){
                exist ? D.resolve("This API Route URL already exists. API Route URL must be unique") : D.resolve();
            });
        }
        return D.promise;
    }
...
Query.executeApiRoute = function (req, res) {
    var env = req.headers.dfx_env_var || 'none';
    if (!req.user) {
        req.user = {};
        req.user.tenantid = req.body.userInfo.tenantId;
        req.user.userid = req.body.userInfo.userId;
    }
    var tenantid = req.user.tenantid || req.user.id || req.params[0],
        apiRoute = ((req.method == 'POST') ? req.body.url : req.query.url) || req.params[0],
    ...
    Query.getDQObjByApiRoute(tenantid, appName, apiRoute).then(function(docObj){
        ...
        if ( docObj === null ) {
            res.status(404).send("Not found apiRoute - " + apiRoute + "!");
            return;
        }
...
function ExecuteQuery(req, res) {
    ...
    // Replace all needed values with Environment variables
    if ( SETTINGS.studio ) {
        mdbw.getOne(DB_TENANTS_PREFIX + this.opts.tenantid, 'environments_map', {app_name: reqParams.application})
            .then(function (response) {
                if (response && response.content) {
                    var envs = response.content.filter(function (env) {
                        return ((env.name === self.opts.env) && (self.opts.env != 'none'));
                    });

                    var envVarsList = [];

                    if (envs[0] && envs[0].data) {
                        envVarsList = envs[0].data;
                    }

                    envVarsList = flatten(envVarsList);

                    for (var key in envVarsList) {
                        self.opts.url = self.opts.url.replace(new RegExp('ENV\\[\\"' + key + '\\"\\]','g'), envVarsList[key]);

                        self.opts.precode.forEach(function (code, index) {
                            self.opts.precode[index].code = self.opts.precode[index].code.replace(new RegExp('ENV\\[\\"' + key + '\\"\\]','g'), envVarsList[key]);
                        });

                        self.opts.postcode.forEach(function (code, index) {
                            self.opts.postcode[index].code = self.opts.postcode[index].code.replace(new RegExp('ENV\\[\\"' + key + '\\"\\]','g'), envVarsList[key]);
                        });

                        self.opts.params.forEach(function (param, index) {
                            self.opts.params[index].value = self.opts.params[index].value.replace(new RegExp('ENV\\[\\"' + key + '\\"\\]','g'), envVarsList[key]);
                        })
                    }
                } else {
                    log.warn("Can't find information about execution environments");
                }

                nextStep(self, reqParams, req, res);

            }).done();
    } else {
        if (req.headers && req.headers.referer) {
            var envFileDir = url.parse(req.headers.referer).pathname.replace('index.html', '').replace('/deploy', '');
            var files = fs.readdirSync(path.join(SETTINGS.deploy_path, envFileDir));
            var envFileName;

            for (var i in files) {
                if (path.extname(files[i]) === ".env") {
                    envFileName = files[i];
                }
            }
            if (envFileName) {
                var envVarsList;
                try {
                    envVarsList = JSON.parse(fs.readFileSync(path.join(path.join(SETTINGS.deploy_path, envFileDir), envFileName), 'utf8'));
                } catch (err) {
                    log.warn("Can't parse JSON from " + path.join(path.join(SETTINGS.deploy_path, envFileDir), envFileName));
                }
            ...
            log.warn("Can't read req.headers.referer");
        }
        nextStep(self, reqParams, req, res);
    }
};

function nextStep(self, reqParams, req, res) {

    req.user.tenantId = self.opts.tenantid;
    req.user.userid = self.opts.userid;

    self.reqParams = self.opts.params;
    self.res       = res;
    self.dialect   = self.parsedUrl = null;
    // make user object accessible to pre/post code
    if (self.opts.precode.length || self.opts.postcode.length) {
        //user_definition.api.getUser({
        //    req: req, userId: req.user.userid || req.session.user.id || self.opts.userid,
        //    tenantId: self.opts.tenantid, applicationName: reqParams.application
        //}).then(function (user) {
        //    console.log("HERE");
        //    console.log(user);
        //    self.opts.user = user;
        ...
        self.opts.user = {
            tenant: self.opts.tenantid,
            roles: { list: [ 'admin' ], default: 'admin' },
            email: '',
            lastName: '',
            firstName: '',
            application: reqParams.application,
            versioning: { status: 'added', user: self.opts.tenantid, last_action: Math.floor(Date.now() / 1000) }
        }
...
Query.set = function (queryName, dataquery, req, callback) {
    ...
    if (!dataquery.change) {
        ...
        dataquery.apiRoutes.forEach(function (apiRoute) {
            if ( !apiRoute.data.uuid ) apiRoute.data.uuid = new Date().getTime();
