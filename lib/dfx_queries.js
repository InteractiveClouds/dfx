...
function ExecuteQuery(req, res) {
    ...
    if ( SETTINGS.studio ) {
        ...
        if (req.headers && req.headers.referer) {
            ...
            var files = old_fs.readdirSync(path.join(SETTINGS.deploy_path, envFileDir));
            var envFileName;
            ...
            if (envFileName) {
                var envVarsList;
                try {
                    envVarsList = JSON.parse(old_fs.readFileSync(path.join(path.join(SETTINGS.deploy_path, envFileDir), envFileName), 'utf8'));
                } catch (err) {
