...
api.getStudioAssets = function(req, res) {
    var assetName = req.params.assetName;
    var app_name = req.query.app_name;
    var tenantId = req.query.tenantId;

    var filePath = path.join(RESOURCES_DEV_PATH, tenantId, app_name, 'assets', assetName);

    QFS.exists(filePath).then(function(exists){
          if (!exists) {
              var filePath = path.join(SETTINGS.public_dir_path, 'images', assetName);
              res.sendFile(filePath);
          } else {
              res.sendFile( path.resolve(path.join(RESOURCES_DEV_PATH, tenantId, app_name, 'assets', assetName)) );
          }
    });
};
...
var getAsset = (function(){
    ...
    function getAppAndTenantFromUrl  (referer, req) {
        ...
        if (referer.indexOf('/studio/widget/') > -1) {
            ...
            if ( !parse ) log.error('can not parse asset\'s referer : ' + referer);
            result = {
                tenantId : parse[1],
                appName  : parse[2]
            }
            ...
            result.appName = temp;
            result.tenantId = util.lastLoginCookie.get(req).tenantid;
        } else if (referer.indexOf('/studio/index.html') > -1) {
            var cookies = parseCookies(req);
            result.appName = cookies['app_name'];
            result.tenantId = cookies['X-DREAMFACE-TENANT'] || util.lastLoginCookie.get(req).tenantid;
        } else {
            // app runtime
    ...
    return function (req, res, isShared) {
        ...
        QFS.exists(filePath).then(function(exists){
            if (!exists) {
                filePath = path.join(SETTINGS.public_dir_path, 'images', assetName);
