...
var DB_TENANTS_PREFIX       = SETTINGS.databases_tenants_name_prefix,
    ...
    APP_RUN_TIME_PATH       = SETTINGS.resources_deploy_path,
...
var getAsset = (function(){
    ...
    function getAppAndTenantFromUrl  (referer, req) {
        ...
        } else if (referer.indexOf('/' + SETTINGS.tempDir + '/') > -1) {
            // page preview
            const PAGE_PREVIEW_PARSE_REFERER = new RegExp("\/" + SETTINGS.tempDir + "\/deploy\/([^\/]+)\/([^\/]+)\/([^\/]+)\/");
