    ...
    sendFile({
        ...
        .then(function(){
                if ( platform=='mobile') {
                    ...
                    var destanationPath =  path.join(__dirname, '..',SETTINGS.tempDir,'appFiles');
                    return unzip.decompress({
                        ...
                        var o = {
                            ...
                            fileName: 'app.zip',
                            outputPath: path.join(__dirname, '..',SETTINGS.tempDir,'app.zip')
                        }
                        return zip.compress(o).then(function () {
                            var o = {
                                ...
                                version: buildNumber,
                                create_method: "file",
                                filePath: path.join(__dirname, '..',SETTINGS.tempDir,'app.zip'),
                                req: {"session": {"tenant": {"id": tenantId}}}
                            }
                            return phoneGap.createApp(o).then(function (response) {
                                var version = buildNumber.split('.');
                                var query = {
                                    application: applicationName,
                                    platform: platform,
                                    app_version: version[0] + '.' + version[1],
                                ...
                                if (response.id) {
                                    return MDBW.update(DB_TENANTS_PREFIX + tenantId, 'application_builds', query, {$set : {"phoneGapAppId" : response.id}}).then(function(){
                                        return QFS.remove(path.join(__dirname, '..',SETTINGS.tempDir,'app.zip')).then(function(){
                                            return rm_r(path.join(__dirname, '..',SETTINGS.tempDir,'appFiles'));
                                        })
                                    });
                                }
...
dep.upload = function(req, res){
    ...
    return tmpDirTool.exec(function(wrkDir){
        ...
        form.parse(req, function(err, fields, files) {
            ...
            unpack({ zipPath: files.file.path})
            .then(function () {
                res.send("OK");
                D.resolve();
            })
