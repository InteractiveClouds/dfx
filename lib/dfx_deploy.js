    ...
    sendFile({
        ...
        .then(function(){
                if ( platform=='mobile') {
                    ...
                    var destanationPath =  path.join(__dirname, '..','tmp','appFiles');
                    return unzip.decompress({
                        ...
                        var o = {
                            ...
                            fileName: 'app.zip',
                            outputPath: path.join(__dirname, '..','tmp','app.zip')
                        }
                        return zip.compress(o).then(function () {
                            var o = {
                                ...
                                version: buildNumber,
                                create_method: "file",
                                filePath: path.join(__dirname, '..','tmp','app.zip'),
                            ...
                            return phoneGap.createApp(o).then(function (response) {
                                ...
                                if (response.id) {
                                    return MDBW.update(DB_TENANTS_PREFIX + tenantId, 'application_builds', query, {$set : {"phoneGapAppId" : response.id}}).then(function(){
                                        return QFS.remove(path.join(__dirname, '..','tmp','app.zip')).then(function(){
                                            return rm_r(path.join(__dirname, '..','tmp','appFiles'));
                                        })
                                    });
                                }
