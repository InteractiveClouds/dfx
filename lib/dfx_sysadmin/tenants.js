...
exports.setTenants = function(req, res) {
    if (req.body.list){
        activeTenants = req.body.list;
        // Logout all tenants not present in list
        console.log(req);
        //require('../auth').studio.loginPage(req, res);
    }
    res.status(200).end();
};
...
exports.list = function() {
    return MDBW.get(sysdbName, tenantsClName)
        .then(function(docs){
            return docs.map(function(e){return e.id})
                .filter(function(f){
    ...
    return validateName(tenantName)
        ...
        .spread(function(exists, token){
            ...
            return Q.all([
                ...
                .then(function(){}) // to return undefined
        })
};
