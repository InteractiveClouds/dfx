...
exports.init = function ( o ) {
    ...
    exports.list().then(function(tenants){
        activator.init(tenants);
...
// SEARCH staffRoles\|staffRights\|rolesCache\|rightsCache
        ...
        function fillCache () {
            ...
            return tenants.list()
                ...
                .then(function(){
                    _cacheIsReady.resolve();
                        return cache.Obj.set("staffRoles",null, staffRoles).then(function(){
                            return cache.Obj.set("staffRights",null, staffRights).then(function(){
                                return cache.Obj.set("rolesCache",null, rolesCache).then(function(){
                                    return cache.Obj.set("rightsCache",null, rightsCache);
                                });
                            });
                        });
                });
        ...
        innerStorage.put = function ( w, name, tenantId ) {
            ...
            return Q.when( cacheIsReady, function(){
                ...
                //return rolesCache[tenantId][w.application][name] = w;

                cache.Obj.set("rolesCache", tenantId + '.' + w.application, rolesCache[tenantId][w.application] || {}).then(function(){
                    return cache.Obj.set("rolesCache", tenantId + '.' + w.application + '.' + name, rolesCache[tenantId][w.application] || {});
                })
            });
        };
        ...
        innerStorage.remove = function (roleName, tenantId, applicationName) {
            ...
            return Q.when( cacheIsReady, function () {
                ...
                //return Q( delete rolesCache[tenantId][applicationName][roleName] );

                return cache.Obj.delete("rolesCache", tenantId + '.' + applicationName + '.' + roleName);
            });
        };
        ...
        innerStorage.update = function ( w, roleName, tenantId, applicationName, userId ) {
            ...
            return Q.when( cacheIsReady, function(){
                ...
                return cache.Obj.set("rolesCache", tenantId + '.' + applicationName + '.' + roleName + '.rights', {}).then(function(){
                    ...
                    if ( w.rights ) {
                        ...
                        w.rights.forEach(function(right){
                            ...
                            cache.Obj.set("rolesCache", tenantId + '.' + applicationName + '.' + roleName + '.rights.right', true)
                        });
                    ...
                    if ( w.hasOwnProperty('description') ) {
                        ...
                        cache.Obj.set("rolesCache", tenantId + '.' + applicationName + '.' + roleName + '.description', w.description);

                        toUpdate.$set.description = w.description;
                    }

                    var filter = applicationName  == sharedCatalogName ? {name: roleName} : {name: roleName, application: applicationName};
            ...
            return cache.Obj.get("rolesCache").then(function (rolesCache){
                ...
                return Q.when(cacheIsReady, function () {
                    ...
                    if (!rolesCache[tenantId][appName][roleName].rights) {
                        ...
                        cache.Obj.set("rolesCache", tenantId + '.' + appName + '.' + roleName + '.rights', {});
                    }
        ...
        innerStorage.changeRightsName = function ( from, to, tenantId, applicationName ) {
            ...
            return cache.Obj.get("rightsCache").then(function(rightsCache) {
                return Q.when(cacheIsReady, function () {
                    ...
                    return cache.Obj.set("rightsCache",tenantId + '.' + applicationName + '.' + to, rightsCache[tenantId][applicationName][from]).then(function(){
                        return cache.Obj.delete("rightsCache",tenantId + '.' + applicationName + '.' + from);
                    });
            ...
            return cache.Obj.get("rightsCache").then(function(rightsCache) {
                return cache.Obj.get("rolesCache").then(function (rolesCache) {
                    return Q.when(cacheIsReady, function () {
                        ...
                        return cache.Obj.set("rightsCache", null, rightsCache).then(function(){
                            return cache.Obj.set("rolesCache", null, rolesCache);
                        })
                    });
                });
            });
        ...
        innerStorage.addTenantToCache = function ( tenantId ) {
            ...
            rolesCache[tenantId] = {};
            rightsCache[tenantId] = {};
            return cache.Obj.set("rolesCache",tenantId, {}).then(function(){
                return cache.Obj.set("rightsCache",tenantId, {});
            })
        };
        ...
        innerStorage.ifTenantExists = function ( tenantId ) {
            console.log("HERE16");
            return Q.when( cacheIsReady, function(){
                cache.Obj.get("rolesCache").then(function(rolesCache){
