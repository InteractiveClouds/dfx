...
ScreenTemplate.createNew = function (screenTemplateParameters, req, callback) {
	var file_suffix = (screenTemplateParameters.platform == 'mobile') ? '_mobile' : '';
    fs.readFile(path.join(__dirname, '..', 'templates/static_json/screentpl/header' + file_suffix + '.html'), 'utf8', function (err, header_tpl) {
        if (err) {
            log.error(err);
        } else {
            screenTemplateParameters.layout.header.content.value = header_tpl;
        }
        fs.readFile(path.join(__dirname, '..', 'templates/static_json/screentpl/footer' + file_suffix + '.html'), 'utf8', function (err_footer, footer_tpl) {
            if (err_footer) {
...
ScreenTemplate.list = function (req, res) {
    ScreenTemplate.selectAll(req.params.applicationName, req, function (arr_screens_templates) {
        res.end(JSON.stringify({
...
ScreenTemplate.selectAll = function (applicationName, req, callback) {
    var filter = {application: applicationName};
    mdbw.select(DB_TENANTS_PREFIX + req.session.tenant.id, 'screens_templates', filter)
        .then(function (docs) {
            callback(docs);
        });
};
