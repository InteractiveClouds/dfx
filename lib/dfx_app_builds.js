...
api.put = function( p ) {
    return MDBW.update(DB_TENANTS_PREFIX + p.tenant, DB_COLLECTION,
        ...
        },
        {
            ...
            deploymentVersion: p.deploymentVersion,
...
api.deploy = function( p ) {
    ...
    return MDBW.update(DB_TENANTS_PREFIX + p.tenant, DB_COLLECTION, searchCriteria, {$set: {deployed: false, deploymentVersion: p.deploymentVersion}})
        .then(function () {
            searchCriteria = {
                application: p.application,
                platform: p.platform,
