...
// return zip file content from app_builds folder
Studio.getBuildFile = function(req, res) {
    if (!req.query.path) {
        res.status(404).end('You need to specify path to zip file into "path" variable');
    } else {
        var obj = path.parse(req.query.path);

        var platform = (path.parse(path.parse(obj.dir).dir)).base;

        var pathToZip = path.join(SETTINGS.app_build_path, obj.dir, obj.base);
        fs.exists(pathToZip, function (exist) {
            if (exist) {
                res.download(pathToZip, platform + '_' + obj.base);
            } else {
                res.status(404).end("file with path - " + pathToZip + " doesn't exists");
            }
        })
    }
...
Studio.settings = function(req, res) {
    res.send(JSON.parse(JSON.stringify(SETTINGS)));
}
