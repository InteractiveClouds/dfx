    ...
    engine.makeTempDirWithTemplates()
        .then(function() {
            return engine.compress({fileName: 'templates.zip'})
                .then(function(){
                    var filePath = path.join(SETTINGS.tempDir, 'templates.zip');
...
engine.getResourcesZip = function(req, res) {
    engine.makeTempDirWithResources(req.query.appname, req.query.tenantid)
        .then(function() {
            engine.compress({fileName: 'resources.zip', folderPath: path.join(SETTINGS.tempDir,'resources')})
                .then(function() {
                    var filePath = path.join(SETTINGS.tempDir,'resources.zip');
                    res.sendFile(filePath);
                });
        }).done();
};
...
engine.makeTempDirWithTemplates = function() {
    return QFS.makeTree( path.join(SETTINGS.tempDirForTemplates ))
        .then(function(){
       return QFS.copyTree(path.join(__dirname,'..','..', 'build'), path.join(SETTINGS.tempDirForTemplates, 'build'))
           .then(function(){
                var templates = SETTINGS.templates;
                   return Q.all(Object.keys(templates).map(function(k){
                       return QFS.makeTree(path.join(SETTINGS.tempDirForTemplates, 'templates',  templates[k].name))
                           .then(function(){
                               return QFS.exists(templates[k].path)
                                   .then(function(res){
                                       return res ? QFS.copyTree(templates[k].path, path.join(SETTINGS.tempDirForTemplates, 'templates',templates[k].name))
                                                  : Q.resolve();
                                   });
                           })
                           .then(function(){
                               return QFS.exists(templates[k].widget)
                                   .then(function(res){
                                       return res ? QFS.copy(templates[k].widget, path.join(SETTINGS.tempDirForTemplates, 'templates', templates[k].name, 'view.jade'))
                                                  : Q.resolve();
                                   });
                           })
                           .then(function(){
                               return QFS.exists(templates[k].login)
                                   .then(function(res){
                                       return res ? QFS.copy(templates[k].login, path.join(SETTINGS.tempDirForTemplates, 'templates', templates[k].name, 'login.jade'))
                                                  : Q.resolve();
                                   });
                           })
                           .then(function(){
                               return QFS.exists(templates[k].index)
                                   .then(function(res){
                                       return res ? QFS.copy(templates[k].index, path.join(SETTINGS.tempDirForTemplates, 'templates', templates[k].name, 'index.jade'))
                                                  : Q.resolve();
                                   });
                           })
                           .then(function(){
                               return QFS.exists(templates[k].screen)
                                   .then(function(res){
                                       return res ? QFS.copy(templates[k].screen, path.join(SETTINGS.tempDirForTemplates, 'templates', templates[k].name, 'page.jade'))
                                                  : Q.resolve();
                                   });
                           })
                   }));
...
engine.makeTempDirWithResources = function(appName, tenantId) {
    var tmpResourcesPath = path.join(SETTINGS.tempDir, 'resources');
...
engine.compress = function(o) {
    ...
    var inputFiles = o.folderPath || path.join(SETTINGS.tempDirForTemplates);

    fs.readdir(inputFiles, function (err, files) {
        if (files.length > 0) {
            var outputPath = o.outputPath || path.join(SETTINGS.tempDir, o.fileName);
