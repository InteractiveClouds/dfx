    ...
    engine.makeTempDirWithTemplates()
        .then(function() {
            return engine.compress({fileName: 'templates.zip'})
                .then(function(){
                    var filePath = path.join(__dirname, '..','..',SETTINGS.tempDir, 'templates.zip');
...
engine.getResourcesZip = function(req, res) {
    engine.makeTempDirWithResources(req.query.appname, req.query.tenantid)
        .then(function() {
            engine.compress({fileName: 'resources.zip', folderPath: path.join(__dirname,'..','..',SETTINGS.tempDir,'resources')})
                .then(function() {
                    var filePath = path.join(__dirname,'..','..',SETTINGS.tempDir,'resources.zip');
                    res.sendFile(filePath);
                });
        }).done();
};
...
engine.makeTempDirWithResources = function(appName, tenantId) {
    var tmpResourcesPath = path.join(__dirname, '..', '..',SETTINGS.tempDir, 'resources');
...
engine.compress = function(o) {
    ...
    var inputFiles = o.folderPath || path.join(__dirname,'..','..',SETTINGS.tempDirForTemplates);

    fs.readdir(inputFiles, function (err, files) {
        if (files.length > 0) {
            var outputPath = o.outputPath || path.join(__dirname,'..','..',SETTINGS.tempDir, o.fileName);
