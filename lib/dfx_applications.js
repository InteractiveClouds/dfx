...
Application.getLogo = function(req, res) {
    ...
    var regex_templates = [/\/deploy\/([^\/]+)\/([^\/]+)\/([^\/]+)\/([^\/]+)\//, /\/widget\/([^\/]+)\/([^\/]+)\/([^\/]+)\/([^\/]+)\//, /\/deploy\/([^\/]+)\/([^\/]+)\/([^\/]+)\//];

    Q(1).then(function(){
        return regex_templates.findIndex(function( template ){
            return (parsed = template.exec(referer) != null)
        })
    }).then(function(index){
        if (index > -1) {
            return {
                parsed : regex_templates[index].exec(referer),
                index : index
            }
        }
    }).then(function(data){
        if (data) {
            if (data.index == 1) {
                ...
                appName = data.parsed[3];
            } else {
                result = {
                    tenantId : data.parsed[1],
                    appName  : data.parsed[2],
                    build    : data.parsed[3],
                    build_number : data.parsed[4]
                }
            ...
            var errorText = 'Can not parse URL! Referer  - ' + referer;
            log.error(errorText);
            res.status(404).send(errorText);
        }
    });
