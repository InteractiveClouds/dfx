...
Application.getLogo = function(req, res) {
    ...
    if (!cookie['X-DREAMFACE-TENANT'] || !cookie['app_name']){
        res.status(404).send("Can't get X-DREAMFACE-TENANT and/or app_name from cookies");
    } else {
        mdbw.getOne(DB_TENANTS_PREFIX + cookie['X-DREAMFACE-TENANT'], 'applications', {'name':cookie['app_name']}).then(function(app){
            res.redirect("/assets/" + app.logo.split('/')[app.logo.split('/').length -1]);
        }).fail(function(err){
