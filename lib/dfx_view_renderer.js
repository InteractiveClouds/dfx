...
renderer.render = function(req) {
 ...
 renderer.renderComponent = function(component_definition) {
    ...
    var file_prefix = component_definition.type == 'json' ? 'gc_json' : component_definition.type;
   ...
   while ((m_cond_attr = conditional_attr_re.exec(expr_template)) !== null) {
	...
	if (attributes.steps) {
		...
		for (var i_wiz=0; i_wiz<attributes.steps.value.length; i_wiz++) {
			...
			wizard_fragment += '<md-tab flex="100" label="{{' + step.label + '}}" ng-disabled="' + step.disabled.value + ' || ' + attributes.steps.value[i_wiz].validDisabled.value + '">' +
			...
			if (i_wiz < (attributes.steps.value.length-1)) {
				wizard_fragment += '<div layout="row" layout-align="end-center" class="dfx-core-gc-wizard-navbuttons">' +
					...
					' ng-class="[' + ((attributes.previousButton.classes.value!='') ? attributes.previousButton.classes.value + ', ' : '') + attributes.previousButton.dynamicClasses.value + ']"' +
					...
					' ng-class="[' + ((attributes.nextButton.classes.value!='') ? attributes.nextButton.classes.value + ', ' : '') + attributes.nextButton.dynamicClasses.value + ']"' +
				...
				wizard_fragment += '<div layout="row" layout-align="end-center">' +
					'<div class="wizard-submit-step dfx-core-gc-wizard-navbuttons" ng-class="is-wizard-submit-step">' +
					'<button ng-click="decrIndex(); ' + attributes.onPrevious.value + '; ' + attributes.previousButton.onclick.value + '; prevent($event);"' +
					' style="' + attributes.previousButton.style.value + '"' +
					' class="dfx-core-gc-wizard-prev md-altTheme-theme glyph md-raised md-primary md-button"' +
					' ng-class="[' + ((attributes.previousButton.classes.value!='') ? attributes.previousButton.classes.value + ', ' : '') + attributes.previousButton.dynamicClasses.value + ']"' +
					...
					' ng-class="[' + ((attributes.submitButton.classes.value!='') ? attributes.submitButton.classes.value + ', ' : '') + attributes.nextButton.dynamicClasses.value + ']"' +
   	...
   	if (attributes.layout || attributes.steps) {
	   	...
	   	while ((m_layout = layout_re.exec(conditional_template)) !== null) {
			...
			if (attributes.layout) {
				//console.log('*************** commented duplicating md-content inside repeatable panel ***************');
				// if (attributes.repeat_in.value != '') {
				// 	layout_fragment += '<div ng-repeat="$dfx_item in ' + attributes.repeat_in.value + '" dfx-repeatable-panel="{{$index}}" layout="column" flex="100">';
				// } else {
   ...
   while ((m_static_attr = re_static.exec(nonv_attribute_template)) !== null) {
	...
	var gc_layout = ((component_definition.type == 'panel' || component_definition.type == 'tabs' || component_definition.type == 'wizard') &&
		(!attributes.autoHeight ||  attributes.autoHeight.value != true)) ?
			' style="height:100%;" flex="100" layout="column" ' : ' layout="column" ';
...
var getTemplateWithReplacedTabsLayout = function(attributes, conditional_template, component_definition) {
    ...
    for (var i_tabs = 0; i_tabs < attributes.tabs.value.length; i_tabs++) {
        ...
        var tab_classes = (tab_value.classes.value == '') ? '' : tab_value.style.value;
        ...
        for (var i_row = 0; i_row < tab_value.layout.rows.length; i_row++) {
            ...
            var row_flex_value = (!attributes.autoHeight || attributes.autoHeight.value != true) ? row.height.value : 'none';
