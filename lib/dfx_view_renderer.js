...
renderer.renderComponent = function(component_definition, parent_layout_column_orientation) {
   	'use strict';
   	/*
     	Rendering Process
   	*/
    var dfx_exec_debug_mode = false;
	...
	if (dfx_exec_debug_mode) console.log('===' + component_definition.type + '(' + component_definition.id + ')===');
	if (dfx_exec_debug_mode) console.log(attributes);
	if (dfx_exec_debug_mode) console.log('============');
	...
	if (component_definition.type=='datatable') {
		...
		for (var dt_col_idx=0; dt_col_idx<attributes.columns.value.length; dt_col_idx++) {
			...
			//if (col.renderer.name=='statictext') {
				datatable_fragment += '<span id="{{component_id}}_column_layout_{{$parent.$index}}_' + dt_col_idx + '" dfx-gcc-renderer="' + col.renderer.name + '" component-id="{{component_id}}" column-id="' + dt_col_idx + '" row-id="{{$parent.$index}}" class="dfx-core-gc-table-column ' + col.columnAlign + '">';
   	...
   	if (attributes.layout || attributes.steps) {
	   	...
	   	while ((m_layout = layout_re.exec(conditional_template)) !== null) {
			...
			if (attributes.layout) {
                var form_fragment = (attributes.form && attributes.form.value != '') ? ' ng-form="' + attributes.form.value + '"' : '';
				layout_fragment += '<div layout="column"' + form_fragment + ' flex="100">';
			}
			var layout_definition = (attributes.layout) ? attributes.layout : attributes.steps.value[layout_index].layout;
			   ...
			   if (dfx_exec_debug_mode) console.log(row);
			   ...
			   for (var i_col=0; i_col<row.cols.length; i_col++) {
                    ...
                    var layout_align = (col.orientation.value ==='row') ? col.halignment.value + ' ' + col.valignment.value : col.valignment.value + ' ' + col.halignment.value;
				   	layout_fragment += '<div id="{{component_id}}_layout_0_row_' + i_row + '_column_' + i_col + '"' + col_flex_width + ' ng-show="' + ((col.display) ? col.display.value : 'true') + '" layout="' + col.orientation.value +
				   		'" layout-wrap layout-align="' + layout_align + '" gc-container="layout_0_row_' + i_row + '_column_' + i_col + '" gc-parent="{{component_id}}" ' + col_style +
					...
					for (var i_child=0; i_child<component_definition.children.length; i_child++) {
						...
						if (child_element.container == 'layout_' + layout_index + '_row_' + i_row + '_column_' + i_col) {
							layout_fragment += renderer.renderComponent(child_element, col.orientation.value);
						}
					}
					layout_fragment += '</div>';
			   }
   ...
   while ((m_attr = re.exec(layout_template)) !== null) {
	 ...
	 try {
       ...
       var attribute_value = eval(attribute).toString();//must be here, otherwise, it will be bytes

	   if (dfx_exec_debug_mode) console.log(attribute, ' / ', attribute_value.toString());
   ...
   while ((m_nonv_attr = re_nonv.exec(attribute_template)) !== null) {
		...
		if (dfx_exec_debug_mode) console.log('ATTR-NONV: ' + nonv_attribute )
   ...
   while ((m_static_attr = re_static.exec(nonv_attribute_template)) !== null) {
		...
		if (dfx_exec_debug_mode) console.log('ATTR-STATIC: ' + static_attribute )
	...
	var gc_layout = getGcLayout(component_definition, parent_layout_column_orientation, attributes, html_fragment);
...
var getGcLayout = function (component_definition, parent_layout_column_orientation, attributes, html_fragment) {
    var gc_layout = '',
        props = {
            height: 'height:100%;',
    ...
    if (component_definition.type == 'panel' || component_definition.type == 'tabs' || component_definition.type == 'wizard') {
        props = adaptToAutoHeight(attributes, props);

        if (parent_layout_column_orientation == 'column') {
            gc_layout = ' style="' + props.height + 'width:' + attributes.flex.value + '%;" flex="100" ' + props.layout;
        } else {
            gc_layout = ' style="' + props.height + '" flex="' + attributes.flex.value + '" ' + props.layout;
        }
    } else if (component_definition.type == 'button') {
        if (attributes.notFlex.value) {
            gc_layout = ' style="flex:0;width:auto;max-width:100%;"';
        } else {
            gc_layout = ' style="width:' + attributes.flex.value + '%;max-width:' + attributes.flex.value + '%;"';
        }
    } else {
        if (attributes.hasOwnProperty('flex')) {
            gc_layout = ' style="width:' + attributes.flex.value + '%;"';
        }
    }
...
var adaptToAutoHeight = function(attributes, props) {
    if (attributes.hasOwnProperty('autoHeight') && attributes.autoHeight.value == true) {
        props.height = '';
        props.layout = '';
    }
    return props;
...
var adaptToRepeatablePanel = function(parent_layout_column_orientation, attributes, props) {
    if (attributes.repeat_in.value != '') {
        if (parent_layout_column_orientation == 'row' ) {
            var number_of_panels = attributes.repeat_in.value.length,
                total_width = attributes.flex.value * number_of_panels,
                min_total_width = total_width > 100 ? 100 : total_width,
                each_rep_panel_width = total_width > 100 ? attributes.flex.value : 100/number_of_panels;
        ...
        } else if (parent_layout_column_orientation == 'column') {
            props.width = 'width:' + attributes.flex.value + '%;';
...
var mergeWithGcTemplates = function (component_template, component_default_attributes) {
    if (component_template && component_template.value !== 'default' && Object.keys(app_gc_templates).length) {
        var gc_template = app_gc_templates[component_template.value];
        if (!gc_template) { return; }
