 ...
 */
 ...
 renderer.renderComponent = function(component_definition) {
   ...
   */
   ...
   var html_template = fs.readFileSync(path.join(__dirname, '..', 'gcontrols/web/'+component_definition.type+'_compile.html')).toString();
   ...
   while ((m = conditional_re.exec(html_template)) !== null) {
     ...
     var conditional_single_re = new RegExp('(' + condition + ')((.|\n)*)(FRAG-ENDIF)');
   ...
   var conditional_attr_re = /(ATTR-IF{{)(.*?)(}})/g;
   ...
   while ((m_cond_attr = conditional_attr_re.exec(html_template)) !== null) {
     ...
     var replace_block_attr = (condition_attr_value=='') ? '' : '' + condition_attr_value + '"';
   ...
   var html_fragment = ''+final_template;
