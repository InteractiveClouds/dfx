	...
	if (component_definition.type=='wizard') {
		...
		for (var i_wiz=0; i_wiz<attributes.steps.value.length; i_wiz++) {
			...
			wizard_fragment += '<md-tab flex="100" label="{{' + step.label + '}}" ng-disabled="' + step.disabled.value + ' || step.validDisabled.value">' +
				...
				'<md-content flex="100" layout="row" style="' + step.style.value + '" ng-class="[' + ((step.classes.value!='') ? step.classes.value + ', ' : '') + step.dynamicClasses.value + ']">' +
			...
			var layout_definition = (attributes.layout) ? attributes.layout : attributes.steps.value[layout_index].layout;
			   ...
			   for (var i_col=0; i_col<row.cols.length; i_col++) {
					var col = row.cols[i_col];
				   	...
				   	var col_style = (col.style.value=='') ? '' : 'style="overflow:auto;' + col.style.value + '" ';
...
var getGcLayout = function (component_definition, parent_layout_column_orientation, attributes, html_fragment) {
    ...
    if (component_definition.type == 'panel' || component_definition.type == 'tabs' || component_definition.type == 'wizard') {
        ...
        if (parent_layout_column_orientation == 'column') {
            gc_layout = ' style="' + props.height + 'width:' + attributes.flex.value + '%;" ' + props.layout;
        } else {
...
var getTemplateWithReplacedTabsLayout = function(attributes, conditional_template, component_definition) {
    ...
    for (var i_tabs = 0; i_tabs < attributes.tabs.value.length; i_tabs++) {
        ...
        layout_fragment += '<md-content flex="100" layout="row" style="height:100%;' + tab_value.style.value +
        ...
        for (var i_row = 0; i_row < tab_value.layout.rows.length; i_row++) {
            ...
            for (var i_col = 0; i_col < row.cols.length; i_col++) {
                ...
                var col_style = ' style="overflow:auto;' + col.style.value + '" ';
                var col_wrap = (col.orientation.value ==='row') ? '\'wrap\'' : '\'nowrap\'';
