	...
	if (component_definition.type=='datatable') {
		...
		for (var dt_col_idx=0; dt_col_idx<attributes.columns.value.length; dt_col_idx++) {
			...
			//if (col.renderer.name=='statictext') {
				datatable_fragment += '<span id="{{component_id}}_column_layout_{{$parent.$index}}_' + dt_col_idx + '" dfx-gcc-renderer="' + col.renderer.name + '" component-id="{{component_id}}" column-id="' + dt_col_idx + '" row-id="{{$parent.$index}}">';
