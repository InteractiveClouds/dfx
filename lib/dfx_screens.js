    ...
    request(url, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            var obj = {
                ...
                dest_path : path.join(__dirname, '..', 'tmp', 'deploy', req.session.tenant.id , req.params.applicationName)
            };
            unzip.decompress(obj)
                .then(function () {
                    var url = 'http://' + SETTINGS.server_host + ':' + SETTINGS.server_port + '/tmp/deploy/' + req.session.tenant.id + '/' + req.params.applicationName + '/app/login.html';
                    replace({
                        regex: '\'index.html\'',
                        replacement: "window.location.href.replace('login.html', '#/page.html?name=" + req.params.screenName + "')",
                        paths: [path.join(__dirname, '..', 'tmp', 'deploy', req.session.tenant.id , req.params.applicationName, 'app', 'login.html')],
                        recursive: true,
                        silent: true
                    });
                    res.send(url);
