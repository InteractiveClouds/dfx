...
Screen.preview = function (req, res) {
    var schemaId = (req.params.platform == 'mobile') ? 'mobileForPreview' : req.params.platform;
    ...
    request(url, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            var obj = {
                path      : path.join(SETTINGS.app_build_path, req.session.tenant.id , schemaId , req.params.applicationName + '_1.0.0' , req.params.applicationName + '_1.0.0.zip'  ),
                dest_path : path.join(SETTINGS.tempDir, 'deploy', req.session.tenant.id , req.params.applicationName)
            };
            unzip.decompress(obj)
                .then(function () {
                    var url = 'http://' + SETTINGS.external_server_host + ':' + SETTINGS.external_server_port + '/' + TMP_DIR_PATH + '/deploy/' + req.session.tenant.id + '/' + req.params.applicationName + '/app/index.html#/' + req.params.screenName;
                    replace({
                        regex: '\'index.html\'',
                        replacement: "window.location.href.replace('login.html', '#/" + req.params.screenName + "')",
                        paths: [path.join(SETTINGS.tempDir, 'deploy', req.session.tenant.id , req.params.applicationName, 'app', 'login.html')],
                        recursive: true,
                        silent: true
                    });
                    res.send(url);
            ...
            res.send("Something went wrong. See server logs for more info. Compiler may not be reachable");
            log.error(url, arguments);
        }
    });
};
