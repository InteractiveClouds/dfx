    ...
    request(url, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            unzip.decompress(obj)
                .then(function () {
                    var url = 'http://' + SETTINGS.external_server_host + ':' + SETTINGS.external_server_port + '/' + TMP_DIR_PATH + '/deploy/' + req.session.tenant.id + '/' + req.params.applicationName + '/app/login.html';
                    replace({
                        regex: '\'index.html\'',
